...
news_search_agent=Agent(
    role ='News Searcher',
    goal="Generate key points for each news articles from the latest news",
    backstory='Expert in analyzing and generating key points from news contents for quick updates',
...
writer_agent= Agent(
    role='Writer',
    goal='Identify all the topic received.Use the get news tool to verify the each topics',
    backstory='Expert in crafting engaging narrative from complex topics',
...
news_search_task = Task(
    ...
    tools=[SearchNewsDB().news],
    expected_output="Key points for each news article"
)
...
writer_task = Task(
    ...
    tools=[GetNews().news, search_tool],
        expected_output="List of key points from the latest news articles related to AI 2024"
